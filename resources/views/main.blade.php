...
<html>
    ...
    <body x-data="{datas:[],week:['日','月','火','水','木','金','土'],type:0,createShow:false}" >
        <div class="w-screen h-screen flex flex-row justify-center  " x-data="{getData(type){axios.get('/api/get?type='+this.type).then(function(response){console.log(response); datas = response.data;});}}" x-init="getData(type); flatpickr('#calendar',{inline:true,'locale':window.Japanese , defaultdate:'today'});">
            ...
            <div class=" w-full flex-78 flex flex-col p-5 overflow-scroll">
                <template class="" x-for="data in datas" >
                    <div class="w-full min-h-12 h-auto border p-2 flex flex-row  shrink-0 @container " :class="data.isFinished ? 'bg-gray-400' : 'bg-gray-200'" x-data="{time:(new Date(data.deadline * 1000)),remainDate:(Math.round(10*((data.deadline * 1000) - (Date.now()+1000*60*60*9))/(1000*60*60*24)))/10}">
                        ...
                        <div class="flex-2 flex items-center flex-row border-x ">
                            ...
                            <p class="text-2xl flex-2 mx-3" x-text="data.isFinished ? '完了!' : '未完了'"></p>

                            <div class="max-w-10 h-10 flex-1">
                        ...
                        <div class="flex-1 h-full flex flex-col justify-center ">
                            ...
                            <div class=" h-7 border bg-white shadow-2xl flex justify-center" @click="axios.post('/api/delete',{'id':data.id}).then(function(response){console.log(response); getData(type);});">削除</div>
                        </div>
            ...
            <div class="h-screen w-screen fixed top-0 left-0 sm:relative sm:h-full sm:w-full sm:flex-22  sm:border-l sm:flex flex-col p-3  bg-white" :class="!createShow && 'hidden'">
                ...
                <div class="w-full ">
                    <form action="/api/create" method="POST" class="flex flex-col" @submit.prevent="axios.post('/api/create',Object.fromEntries(new FormData($event.target))).then(function(response){console.log(response); getData(type);}) ;">
